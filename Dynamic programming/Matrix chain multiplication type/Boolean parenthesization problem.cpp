...
i and j (both inclusive) such that the subexpression between i and j evaluates to true.

T(i,j) = summation(k=i to j-1){ T(i,k)*T(k+1,j) 							if operator[k] is '&' }
							  { Total(i,k)*Total(k+1,j) - F(i,k)*F(k+1,j)   if operator[k] is '|' }
							  { T(i,k)*F(k+1,j) + F(i,k)*T(k+1,j)           if operator[k] is '^' }
where, Total(i,j) = T(i,j) + F(i,j)

Let F(i, j) represents the number of ways to parenthesize the symbols between 
i and j (both inclusive) such that the subexpression between i and j evaluates to false.

F(i,j) = summation(k=i to j-1){ Total(i,k)*Total(k+1,j) - T(i,k)*T(k+1,j)	if operator[k] is '&' }
