...
Capital, LeetCode would like to work on some projects to increase its capital before the IPO. Since it 
has limited resources, it can only finish at most k distinct projects before the IPO. Help LeetCode 
...
0 <= capital[i] <= 10^9
*/
// greedily choose the max profits whose capital <= w
class Solution {
public:
    int findMaximizedCapital(int k, int w, vector<int>& profits, vector<int>& capital) {
        priority_queue<array<int,2>, vector<array<int,2>>, greater<array<int,2>>> minCapital;
        ...
        for (int i = 0; i < k; ++i) {
            while (!minCapital.empty() and minCapital.top()[0] <= w) {
                auto top = minCapital.top();
                minCapital.pop();
                maxProfits.push(top[1]);
            }
