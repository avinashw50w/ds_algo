...
// returns the sum of subree rooted at root if the subtree satisfies sumTree property
// otherwise returns -1
...
int solve(Node *root) {
  ...
  if (ls == -1 or rs == -1) return -1;

  return (root->data == ls + rs) ? root->data + ls + rs : -1;
}

int main() {
  if (solve(root) == -1) cout << "The tree is a sumtree\n";
  else cout << "The tree is not a sumtree\n";
}
/////////////////////////////////////////////////////////
